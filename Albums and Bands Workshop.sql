...
VALUES ('Iron maid'),('Deuce'),('Avenged Sevenfold'),('Ankor');

SELECT * FROM Bands;
SELECT * FROM Bands LIMIT 2;
SELECT NAME FROM BANDS;
SELECT name as 'Band Authors' FROM Bands;
SELECT * FROM Bands ORDER BY name;

INSERT INTO Albums(name, release_year, band_id)
VALUES  ('The No.of the Beast', 1985, 1),
		('Power Slave',1984, 1),
		('Nightmare',2018,2),
        ('Nightmare',2010,3),
        ('Test Album',Null, 3);
...
select name from albums order by name DESC LIMIT 3;
...
SELECT * FROM ALBUMS WHERE release_year > 2010;
SELECT * FROM ALBUMS WHERE release_year BETWEEN 1982 AND 2010;
SELECT * FROM ALBUMS WHERE release_year NOT BETWEEN 1984 AND 2010;
SELECT * FROM ALBUMS WHERE (release_year >= 1984 AND release_year <= 2010); 
SELECT * FROM ALBUMS WHERE (release_year > 2010 OR release_year < 1984);
...
SELECT B.name as Band_Name, COUNT(A.id) as Num_of_Albums
FROM Bands as B JOIN Albums as A
ON B.id = A.band_id
GROUP BY A.band_id
HAVING Num_of_Albums = 1;
...
SELECT * FROM BANDS;
INSERT INTO Bands( name)
VALUES ('King of Snakes');

INSERT INTO Albums(name, release_year, band_id)
VALUES ('The Broken Heart', 2021, 8);

Select bands.name as Band_Name, albums.id as Album_id, albums.name as Album_Name, albums.release_year as Release_Year, albums.band_id as Band_ID 
from Bands Inner Join Albums 
ON bands.id = albums.band_id 
where bands.id = 8;
...
SELECT a.name, a.release_year, max(s.length) as 'Duration'
FROM Albums AS a 
JOIN Songs AS s
ON a.id = s.album_id
Group by a.id;
